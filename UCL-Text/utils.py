...
def text_preprocess(x, y, use_gpu=False, mode="onehot"):
    if mode == "onehot":
        # x are indices
        x = torch.LongTensor(x)
    else:
        # x are embedding features extracted by BERT
        x = torch.Tensor(x)
...
def predict(model, x):
    ...
    for i in range(num_all_batch):
        with torch.no_grad():
            ...
            x_batch = x[batch_idx]
            if x_batch.shape[0] != len(batch_idx):
...
def train(model,
    ...
    for epoch in range(num_epoch):
        ...
        for idx in range(num_all_tr_batch):
            ...
            if x_batch.shape[0] != len(batch_idx):
                # when there is only single data
                x_batch = x_batch.unsqueeze(0)
